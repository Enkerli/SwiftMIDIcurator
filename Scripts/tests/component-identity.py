#!/usr/bin/env python3
"""Checks SwiftMIDIcurator's audio component identity.
...
This plug-in was scaffolded by copying SwiftPitchFold's project file, which came
from Serpe's, which came from ProgGenie's, which came from MelGen's, so it
started life claiming somebody else's triple four removes back. It is
`aumi/MdCr/Enke` — deliberately *not* `Mcur`, which belongs to the JUCE
MIDIcurator and would collide with the AUv3 that build already ships.

The bundle identifier is the other forever identifier, and it is the one that
bit: the Swift Serpe shipped as `com.enkerli.Serpe` against the JUCE
`com.enkerli.serpe` and **did not appear in AUM at all**. This repo is
`SwiftMIDIcurator` and `com.enkerli.SwiftMIDIcurator` for exactly that reason —
the name is not decoration, it is what keeps the two installable side by side.
...
Checks:
  ...
  3. The display name follows the "Manufacturer: Product" convention.
  4. The bundle identifier doesn't collide either — a different failure with a
     worse symptom, since the plug-in then does not appear at all.
  5. CLAUDE.md and README.md quote the same triple the plist declares.
...
INFO_PLIST = REPO / "SwiftMIDIcuratorExtension" / "Info.plist"
PROJECT = REPO / "SwiftMIDIcurator.xcodeproj" / "project.pbxproj"
HOST_MODEL = REPO / "SwiftMIDIcurator" / "Model" / "AudioUnitHostModel.swift"
